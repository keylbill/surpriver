...
class Surpriver:
    ...
    def send_telegram_message(self, message):
        bot_token = '6677044888:AAHiXihBV3EYqVbA7H1nJ-b7IicgiulfeDk'
        chat_id = '-4169430358'
    ...
    def analyze_and_notify(self, predictions_with_output_data):
        """
        Analyzes the anomaly detection results and sends notifications.
        """
        # Get today's date in the format 'MM-DD-YYYY'
        today_date = datetime.datetime.today().strftime('%m-%d-%Y')

        if not predictions_with_output_data:
            print("No data available for analysis.")
            self.send_telegram_message("No data available for anomaly detection today.")
            return

        # Filter for negative anomaly scores
        negative_anomalies = [item for item in predictions_with_output_data if item[0] < 0]
        if negative_anomalies:
            message = f"Anomalous Stocks Report for {today_date}:\n\n"
            message = "\n".join([f"Symbol: [{symbol}](https://robinhood.com/stocks/{symbol}), Anomaly Score: {score:.3f}" for score, symbol, _, _ in negative_anomalies])
        else:
            message = f"No anomalous stocks for today, {today_date}!"

        # Ensure message does not exceed Telegram's character limit
        if len(message) > 4096:
    ...
    def find_anomalies(self):
        ...
        # Analyze and send notification
        self.analyze_and_notify(predictions_with_output_data)
